...
ROOT_URLCONF = 'Effio_Ielts.urls' # Main urls file

TEMPLATES = [
    {
        'BACKEND': 'django.template.backends.django.DjangoTemplates',
        'DIRS': [
            BASE_DIR / 'Homepage' / 'templates',  # Add this line for app-specific templates
            BASE_DIR / 'templates',               # Add this line for global templates
        ],
        'APP_DIRS': True,
        'OPTIONS': {
            'context_processors': [
...
WSGI_APPLICATION = 'Effio_Ielts.wsgi.application' # WSGI application path for deployment 
...
TIME_ZONE = 'Asia/Ho_Chi_Minh' # Set your timezone accordingly ( Vietnam)
...
STATIC_URL = config('STATIC_URL', default='/static/') # URL to access static files
STATICFILES_DIRS = [
    BASE_DIR / "static", # Directory for static files in development
]
STATIC_ROOT = config('STATIC_ROOT', default=os.path.join(BASE_DIR, 'staticfiles')) # Directory where static files will be collected for production
...
ACCOUNT_LOGIN_METHODS = {'email', 'username'}  # Allow login via email or username
...
# CSRF Settings 
CSRF_COOKIE_SECURE = config('CSRF_COOKIE_SECURE', default=False, cast=bool) # Ensure CSRF cookies are only sent over HTTPS (production)
SESSION_COOKIE_SECURE = config('SESSION_COOKIE_SECURE', default=False, cast=bool) # Ensure cookies are only sent over HTTPS (production)
